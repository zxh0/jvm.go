package instructions

import "jvmgo/rtda"

// Store double into local variable 
type dstore struct {
    index uint8
}
func (self *dstore) fetchOperands(bcr *BytecodeReader) {
    self.index = bcr.readUint8()
}
func (self *dstore) execute(thread *rtda.Thread) {
    _dstore(thread, uint(self.index))
}

type dstore_0 struct {}
func (self *dstore_0) fetchOperands(bcr *BytecodeReader) {}
func (self *dstore_0) execute(thread *rtda.Thread) {
    _dstore(thread, 0)
}

type dstore_1 struct {}
func (self *dstore_1) fetchOperands(bcr *BytecodeReader) {}
func (self *dstore_1) execute(thread *rtda.Thread) {
    _dstore(thread, 1)
}

type dstore_2 struct {}
func (self *dstore_2) fetchOperands(bcr *BytecodeReader) {}
func (self *dstore_2) execute(thread *rtda.Thread) {
    _dstore(thread, 2)
}

type dstore_3 struct {}
func (self *dstore_3) fetchOperands(bcr *BytecodeReader) {}
func (self *dstore_3) execute(thread *rtda.Thread) {
    _dstore(thread, 3)
}

func _dstore(thread *rtda.Thread, index uint) {
    val := thread.CurrentFrame().OperandStack().PopDouble()
    thread.CurrentFrame().LocalVars().SetDouble(index, val)
}
